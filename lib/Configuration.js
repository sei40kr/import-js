...
export default class Configuration {
  ...
  loadUserConfig(): ?Object {
    const explorer = cosmiconfig('importjs', {
      packageProp: false,
      js: '.[moduleName].js',
      sync: true,
    });

    const userConfig = explorer.load(this.workingDirectory);

    if (!userConfig) {
      return null;
    }
    if (Object.keys(userConfig).length === 0) {
      throw new Error('Nothing exported from .importjs.js You need to use `module.exports` to specify what gets exported from the file.');
    }
